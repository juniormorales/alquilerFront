...
import { RechazarSolicitudComponent } from 'src/app/pages/perfiles/arrendero/modulos/solicitudes/rechazar-solicitud/rechazar-solicitud.component';
...
export class ModalVerDetalleSolService {
  ...
  modalEscribirRechazo(obj: ISolicitudPropiedad): Observable<any> {
    const config: ModalOptions = {
      initialState:  {
        input_sol_prop: obj
      },
    ...
    this.modalRef = this.bsModalService.show(RechazarSolicitudComponent, config);
    return new Observable<any>(observer => {
      const subscripcion = this.bsModalService.onHidden.subscribe((reason: any) => {
        if(reason === "CERRAR"){
          observer.error();
